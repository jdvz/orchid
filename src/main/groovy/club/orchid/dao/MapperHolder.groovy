...
class MapperHolder {
    ...
    public <T extends Page<T>> RowMapper<T> getOrCreatePageMapper(String className) {
        final Package pagePackage = club.orchid.domain.cms
        final Class<T> pageClass = this.class.classLoader.loadClass("${pagePackage.name}.${className}", true)
        return getOrCreateMapper(pageClass)
    }
...
//            final RowMapper<T> newMapper = this.class.classLoader.loadClass("club.orchid.dao.mapper.${clazz.simpleName}Mapper", true )?.newInstance()
            final Class<T> pageMapperClazz = this.class.classLoader.loadClass("club.orchid.dao.mapper.${clazz.simpleName}Mapper", true )
